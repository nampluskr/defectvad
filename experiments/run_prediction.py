...
SCRIPT_FILE = os.path.join(PROJECT_DIR, "experiments", "predict.py")
    ...
    for dataset in dataset_list:
        ...
        for model in model_list:
            for category in category_list[dataset]:
                ...
                if SAVE_NORMAL:  cmd.extend(["--save_normal"])
                if CALIBRATE:    cmd.extend(["--calibrate"])
                if IMAGE_LEVEL:  cmd.extend(["--image_level"])
